...
if args.input:
    for link in args.input:
        ...
        t1.start()
        comic = RCO_Comic()
        issues_links = list(comic.get_issues_links(link))
        ...
        issues_id = [comic.get_comic_and_issue_name(
            link) for link in issues_links]
        downloaded_issues = filter(comic.is_comic_downloaded, issues_id)
...
if args.single:
    ...
    for issue_link in args.single:
        ...
        comic = RCO_Comic()
        issue_name = comic.get_comic_and_issue_name(issue_link)
